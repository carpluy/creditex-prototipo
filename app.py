...
section[data-testid="stSidebar"] {{
    ...
    box-shadow: 5px 0 20px rgba(0,0,0,.04) !important;
...
section[data-testid="stSidebar"] .stButton>button {{
    ...
    box-shadow: 0 4px 12px rgba(108,59,255,.25) !important;
}}
section[data-testid="stSidebar"] .stSelectbox>div>div {{
    background: {BG_APP} !important; border:1px solid {BORDER} !important;
    border-radius: 10px !important; color: {TEXT_MAIN} !important;
}}

/* HEADER */
.main-header {{
    background: linear-gradient(135deg, {C_PRIMARY} 0%, #4A56FF 50%, {C_SECONDARY} 100%);
...
/* MÉTRICAS nativas (tabs análisis) */
...
[data-testid="metric-container"]:hover {{ transform:translateY(-3px) !important; box-shadow:0 10px 25px rgba(0,0,0,.08) !important; }}
[data-testid="stMetricLabel"] {{ color:{TEXT_LIGHT} !important; font-size:15px !important; font-weight:600 !important; }}
[data-testid="stMetricValue"] {{ color:{TEXT_MAIN} !important; font-size:42px !important; font-weight:800 !important; }}
...
[aria-selected="true"] {{
    ...
    box-shadow: 0 4px 12px rgba(108,59,255,.35) !important;
}}
...
.stButton>button {{
    ...
    box-shadow: 0 4px 14px rgba(108,59,255,.3) !important;
    transition: all .2s !important;
}}
.stButton>button:hover {{
    transform: translateY(-2px) !important;
    box-shadow: 0 8px 22px rgba(108,59,255,.45) !important;
}}
...
/* FOOTER */
.footer {{ text-align:center;padding:18px;font-size:13px;color:{TEXT_LIGHT};border-top:1px solid {BORDER};margin-top:24px; }}
...
with st.expander("☁️ Conexión a base de datos en la nube — CREDITEX", expanded=not st.session_state.cloud_connected):
    ...
    with col_cfg:
        st.markdown(f"""<div style="display:flex;gap:24px;flex-wrap:wrap;font-size:14px;color:{TEXT_LIGHT};margin:8px 0">
...
with st.sidebar:
    ...
    st.markdown(f"<div style='font-size:13px;color:{TEXT_LIGHT}'>☁️ BD: <b style='color:{TEXT_MAIN}'>{CLOUD_CONFIG['database']}</b></div>", unsafe_allow_html=True)
    st.markdown(f"<div style='font-size:13px;color:{TEXT_LIGHT}'>📊 {len(df_raw):,} registros cargados</div>", unsafe_allow_html=True)
    st.markdown(f"<div style='font-size:13px;color:{TEXT_LIGHT}'>🤖 Precisión: <b style='color:{C_PRIMARY}'>{precision*100:.1f}%</b></div>", unsafe_allow_html=True)

# ── PREDICCIÓN ────────────────────────────────────────────────────────────────
ent={"Temperatura":temperatura,"Humedad":humedad,"RPM_Telar":rpm,
     ...
     "Tiempo_Reparacion_Min":t_reparac,"Dias_Desde_Mantenimiento_Preventivo":dias_mant,
     "Turno_A":int(turno=="A"),"Turno_B":int(turno=="B"),"Turno_C":int(turno=="C")}
prob=modelo.predict_proba(pd.DataFrame([ent])[columnas])[0][1]
riesgo_pct=prob*100
if riesgo_pct>=55:   nivel,color_g,emoji,badge,gauge_color="ALTO",C_DANGER,"🔴","bg-red","#F87171"
elif riesgo_pct>=35: nivel,color_g,emoji,badge,gauge_color="MEDIO",C_WARNING,"🟡","bg-yellow","#FACC15"
else:                nivel,color_g,emoji,badge,gauge_color="BAJO",C_SUCCESS,"🟢","bg-green","#4ADE80"

PLT=dict(paper_bgcolor=BG_CARD,plot_bgcolor=BG_CARD,
         font=dict(color=TEXT_MAIN,size=14),
         xaxis=dict(gridcolor=BORDER,color=TEXT_LIGHT),
         yaxis=dict(gridcolor=BORDER,color=TEXT_LIGHT))
...
with tab1:
    ...
    with c1:
        st.markdown(f"""<div class="kpi-card">
          ...
          <p class="kpi-value" style="color:{color_g}">{riesgo_pct:.1f}%</p>
    ...
    with c2:
        st.markdown(f"""<div class="kpi-card">
          ...
          <p class="kpi-value">{temperatura:.1f} °C</p>
    ...
    with c3:
        st.markdown(f"""<div class="kpi-card">
          ...
          <p class="kpi-value">{rpm}</p>
          <span class="kpi-badge bg-green">✔ Óptimo</span></div>
        </div>""", unsafe_allow_html=True)
    with c4:
        ef_badge="bg-green" if eficiencia>=80 else "bg-yellow"
        ef_text="✔ Bueno" if eficiencia>=80 else "⚠ Revisar"
        st.markdown(f"""<div class="kpi-card">
          <div class="kpi-icon ic-green">📊</div>
          <div><p class="kpi-label">Eficiencia</p>
          <p class="kpi-value">{eficiencia:.1f}%</p>
          <span class="kpi-badge {ef_badge}">{ef_text}</span></div>
        </div>""", unsafe_allow_html=True)
    ...
    with cg:
        st.markdown(f'<p class="section-title">📍 Indicador de riesgo</p>', unsafe_allow_html=True)
    ...
    with ca:
        st.markdown(f'<p class="section-title">🔔 Estado del proceso</p>', unsafe_allow_html=True)
        if nivel=="ALTO":
        ...
        st.markdown(f'<p class="section-title">📋 Parámetros del telar actual</p>', unsafe_allow_html=True)
        st.markdown(f"""<table class="params-table">
          ...
          <tr><td>⚙️ RPM Telar</td><td><b>{rpm}</b></td></tr>
          <tr><td>📊 Eficiencia</td><td><b>{eficiencia} %</b></td></tr>
          <tr><td>🧵 Roturas urdimbre</td><td><b>{roturas_urd}</b></td></tr>
          <tr><td>🧵 Roturas trama</td><td><b>{roturas_tram}</b></td></tr>
          <tr><td>🛠 Días mantenimiento</td><td><b>{dias_mant}</b></td></tr>
        </table>""", unsafe_allow_html=True)

    st.markdown(f'<p class="section-title" style="margin-top:28px">📈 Relación RPM vs Nivel de Riesgo — datos reales CREDITEX</p>', unsafe_allow_html=True)
    ...
    fig2.update_layout(height=320,showlegend=False,margin=dict(t=10,b=10),**PLT)
...
with tab2:
    ...
    if eficiencia<75:  recs.append(("📊 Eficiencia baja",f"{eficiencia:.1f}%. Telares <75% tienen mayor tasa de rechazo.","alta"))
    if t_parada>120:   recs.append(("⏸ Tiempo de parada excesivo",f"{t_parada} min. Investigar causa raíz.","media"))
    if dias_mant>60:   recs.append(("🛠 Mantenimiento vencido",f"{dias_mant} días. Programar mantenimiento urgente.","alta"))
    ...
    st.markdown(f'<p class="section-title" style="margin-top:24px">📊 Distribución de acciones recomendadas</p>', unsafe_allow_html=True)
    ...
    fig_acc=px.bar(acc_df,x="Frecuencia",y="Acción",orientation="h",
                   color="Frecuencia",color_continuous_scale=[[0,"#a29bfe"],[1,C_PRIMARY]])
    fig_acc.update_layout(height=260,coloraxis_showscale=False,margin=dict(t=10,b=10),**PLT)
...
with tab3:
    ...
    k2.metric("⚠️ Nivel alto de riesgo",f"{summ['pct_nivel_alto']}%")
    ...
    with ca2:
        st.markdown(f'<p class="section-title">🏆 Variables más influyentes</p>', unsafe_allow_html=True)
        ...
        fig_i=px.bar(top,x="Importancia",y="Variable",orientation="h",
                     color="Importancia",color_continuous_scale=[[0,"#a29bfe"],[1,C_PRIMARY]])
        fig_i.update_layout(height=360,coloraxis_showscale=False,margin=dict(t=10,b=10),**PLT)
        st.plotly_chart(fig_i,use_container_width=True)
    with cb2:
        st.markdown(f'<p class="section-title">👷 Nivel de riesgo por turno</p>', unsafe_allow_html=True)
        ...
        fig_t.update_layout(height=360,margin=dict(t=10,b=10),**PLT)
        st.plotly_chart(fig_t,use_container_width=True)

    cc2,cd2=st.columns(2)
    with cc2:
        st.markdown(f'<p class="section-title">🔍 Tipo de defecto más frecuente</p>', unsafe_allow_html=True)
        def_df=df_raw["Tipo_Defecto"].value_counts().reset_index(); def_df.columns=["Tipo","n"]
        fig_d=px.pie(def_df,values="n",names="Tipo",
                     color_discrete_sequence=[C_PRIMARY,C_SUCCESS,C_DANGER,C_WARNING,"#a29bfe","#00cec9"])
        fig_d.update_layout(height=340,paper_bgcolor=BG_CARD,font={"color":TEXT_MAIN,"size":13},margin=dict(t=10,b=10))
        st.plotly_chart(fig_d,use_container_width=True)
    with cd2:
        st.markdown(f'<p class="section-title">📊 Eficiencia vs nivel de riesgo</p>', unsafe_allow_html=True)
        fig_e=px.violin(df_raw,x="Nivel_Riesgo",y="Eficiencia_Telar",color="Nivel_Riesgo",
                        color_discrete_map={"Alto":C_DANGER,"Medio":C_WARNING,"Bajo":C_SUCCESS},
                        category_orders={"Nivel_Riesgo":["Bajo","Medio","Alto"]})
        fig_e.update_layout(height=340,showlegend=False,margin=dict(t=10,b=10),**PLT)
        st.plotly_chart(fig_e,use_container_width=True)

    st.markdown(f'<p class="section-title">🏢 Resultado de inspección por cliente</p>', unsafe_allow_html=True)
    ...
    fig_cli.update_layout(height=320,margin=dict(t=10,b=10),**PLT)
...
st.markdown(f"""<div class="footer">
  🧵 <b>UNI</b> · <b>CREDITEX S.A.A.</b> · ☁️ Azure SQL Database · 5,000 registros reales · Prototipo v4.0 · <b>CONCYTEC 2026</b>
</div>""", unsafe_allow_html=True)
